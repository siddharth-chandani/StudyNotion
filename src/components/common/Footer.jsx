          ...
          <div className="w-full lg:w-[50%] flex flex-wrap lg:flex-row justify-between lg:border-r lg:border-richblack-700 pl-3 lg:pr-5 gap-3">
            <div className="w-full lg:w-[30%] mb-7 lg:pl-0">
              ...
              <div className="flex flex-col gap-2 mt-2">
                {["About", "Careers", "Affiliates"].map((ele, i) => (
                  ...
                  >
                    <Link className="hover:cursor-pointer" to={`/${ele.toLowerCase()}`}>{ele}</Link>
                  </div>
                ))}
              </div>
            ...
            <div className="w-full lg:w-[30%] mb-7 lg:pl-0">
              ...
              <div className="flex flex-col gap-2 mt-2">
                {Resources.map((ele, index) => (
                  <div
                    ...
                    className="text-[14px] hover:text-richblack-50 transition-all duration-200"
                  >
                    <Link className="hover:cursor-pointer"  to={`/${ele.split(" ").join("-").toLowerCase()}`}>{ele}</Link>
                  </div>
                ))}
              </div>
              ...
              <div className="text-[14px] hover:text-richblack-50 transition-all duration-200 mt-2">
                <Link className="hover:cursor-pointer"  to="/help-center">Help Center</Link>
              </div>
            </div>
            ...
            <div className="w-full lg:w-[30%] mb-7 lg:pl-0">
              ...
              <div className="flex flex-col gap-2 mt-2">
                {Community.map((ele, index) => (
                  ...
                  >
                    {ele.external ? (
                      <a className="hover:cursor-pointer" href={ele.link} target="_blank" rel="noopener noreferrer">
                        {ele.name}
                      </a>
                    ) : (
                      <Link className="hover:cursor-pointer"  to={ele.link}>{ele.name}</Link>
                    )}
                  </div>
                ))}
              </div>
                ...
                <div className="flex flex-col gap-2 mt-2">
                  {ele.links.map((link, index) => (
                    <div
                      ...
                      className="text-[14px] hover:text-richblack-50 transition-all duration-200"
                    >
                      <Link className="hover:cursor-pointer"  to={link.link}>{link.title}</Link>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <div className="flex justify-between lg:items-start items-center flex-col lg:flex-row gap-3 w-full">
          <div className="flex flex-row flex-wrap">
            {BottomFooter.map((ele, i) => (
              ...
              >
                <Link className="hover:cursor-pointer"  to={`/${ele.split(" ").join("-").toLocaleLowerCase()}`}>
                  {ele}
                </Link>
              </div>
            ))}
